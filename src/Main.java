public class Main {
    ...
    public static String camelCaseMe(String str) {
        String thing = ""; //I call it thing just cuz IDK but it helps my brain :)
        char firstLet = str.charAt(0);
        if (firstLet != '-' && firstLet != '_') {
            thing = thing + firstLet;
        }
        for (int i = 1; i < str.length(); i++) {
            if (str.charAt(i) != '-' && str.charAt(i) != '_') {
                if (str.charAt(i-1) != '-' && str.charAt(i-1) != '_') {
                    ...
                    thing += str.substring(i,i+1).toUpperCase();
                }
            }
        }
        return thing;
